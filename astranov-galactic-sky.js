// === GALACTIC SKY — real constellations + exoplanet host stars (not solar system) ===
const GalacticSky = {
  SKY_R: 2.05,
  _inited: false,
  ...
  EXO_HOSTS: [
    ...
    { name: '55 Cancri', star: '55 Cancri A', ra: 8.7484, dec: 28.662, dist: 41, planets: '5 planets · f rocky', color: 0xaaddff },
    ...
    { name: 'K2-18', star: 'K2-18', ra: 11.5577, dec: 7.590, dist: 124, planets: 'K2-18b · water vapor', color: 0x44ddaa },
    { name: 'LHS 1140', star: 'LHS 1140', ra: 0.5953, dec: -15.310, dist: 40.7, planets: 'LHS 1140b', color: 0xcc88ff },
    { name: 'HD 209458', star: 'HD 209458', ra: 22.0940, dec: 18.884, dist: 159, planets: 'Osiris · first transit', color: 0xff88cc },
    { name: 'WASP-12', star: 'WASP-12', ra: 6.5914, dec: 29.091, dist: 1400, planets: 'WASP-12b · hot Jupiter', color: 0xff5522 },
    { name: 'TOI-700', star: 'TOI-700', ra: 6.7717, dec: -62.601, dist: 101, planets: 'TOI-700d · hab zone', color: 0x66ffcc },
    { name: 'Barnard', star: "Barnard's Star", ra: 17.9548, dec: 4.668, dist: 5.96, planets: 'b · super-Earth', color: 0xddaa66 },
    { name: 'Wolf 359', star: 'Wolf 359', ra: 10.2787, dec: 7.000, dist: 7.86, planets: 'b candidate', color: 0xff9966 },
    { name: 'GJ 1214', star: 'GJ 1214', ra: 17.5694, dec: 4.960, dist: 48, planets: 'GJ 1214b · mini-Neptune', color: 0x88ddff },
  ...
  guideHtml(level, camZ) {
    ...
    if (level === 'orbit') {
      html = '<div class="cg-title">Near-Earth orbit</div>'
        + '<div class="cg-item"><b>ISS</b> — live crew station · ~400 km</div>'
        + '<div class="cg-item"><b>LEO shells</b> — broadband orbital rings</div>'
        + '<div class="cg-item"><b>Constellations</b> — real RA/Dec · your horizon</div>';
    } else if (level === 'galactic') {
      html = '<div class="cg-title">Galactic sky · confirmed exoplanet hosts</div>'
        + '<div class="cg-item"><i>Real equatorial positions · not toy solar orbits</i></div>';
      this.EXO_HOSTS.slice(0, 8).forEach(h => {
        html += '<div class="cg-item"><b>' + h.star + '</b> — ' + h.planets
          + ' <i>· ' + h.dist + ' ly</i></div>';
      });
      html += '<div class="cg-item"><i>+' + (this.EXO_HOSTS.length - 8) + ' more hosts · type stars for full nav</i></div>';
    } else if (level === 'galaxy') {
      html = '<div class="cg-title">Milky Way context</div>'
        + '<div class="cg-item"><b>Exoplanet hosts</b> — ' + this.EXO_HOSTS.length + ' confirmed systems marked</div>'
        + '<div class="cg-item"><b>Star field</b> — spiral arm sampling · zoom in for sky dome</div>'
        + '<div class="cg-item"><i>Scroll in → galactic sky → constellations → earth</i></div>';
    ...
    if (_sync) {
      GN._syncChip = function() {
        ...
        if (chip) {
          ...
          if (cosmic === 'galaxy') chip.textContent = 'GALAXY · scroll in → exoplanet hosts → earth';
          else if (cosmic === 'galactic') chip.textContent = 'GALACTIC SKY · real exoplanet star positions';
          else if (cosmic === 'orbit') chip.textContent = 'ORBIT · constellations · scroll out → galactic sky';
        }
        return _sync();
      };
    }
