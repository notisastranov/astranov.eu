// === SUPERBOOKING PROVISION — instant {slug}.astranov.eu web presence ===
const SuperBookingProvision = {
  ...
  parseAsk(text) {
    ...
    const m = raw.match(/(?:create|make|start|open|provision|build)\s+(?:my\s+)?(?:site|shop|page|presence|profile|booking|superbook(?:er|ing)?)\s*(?:at|on|for)?\s*([a-z0-9][a-z0-9-]{1,30}[a-z0-9])?/i)
    ...
    if (!slug) {
      const words = raw.replace(/\.astranov\.eu/gi, '').split(/\s+/).filter(w => w.length > 2 && !/^(my|the|a|an|for|at|on|create|make|site|shop|page)$/i.test(w));
  ...
  async provision(opts = {}) {
    if (!Auth?.user) {
      Auth?.signInGoogle?.();
      throw new Error('Sign in (G) to create your site.');
    }
  ...
  _onLive(result, opts = {}) {
    ...
    FieldBrain?.pulse?.('commerce', 'site live · ' + result.domain, { role: 'client', props: { site_id: result.site_id } });
    AciCli?.print('SuperBooking live → ' + url, 'ok');
    ACIControl?.reply('Your site is live: ' + url);
  ...
  async cli(parts) {
    ...
    return { error: 'usage: book create <slug> [name] | book list' };
  },
};
window.SuperBookingProvision = SuperBookingProvision;