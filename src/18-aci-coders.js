...
const AciCoders = {
  ...
  async ensureBridge() {
    ...
    if (AciCli) {
      ...
      AciCli.print('  if Composer is down → auto fallback LLM (you will be told)', 'dim');
    }
    this.updateHud();
  ...
  startPoll(summonId) {
    ...
    this._pollTimer = setInterval(async () => {
      ...
      if (tries === 12 && r?.pending && AciCli) {
        AciCli.print('Composer still working… #' + summonId + ' (poll continues)', 'dim');
      }
      if (tries > 36) {
        this.stopPoll();
        if (AciCli) AciCli.print('Composer slow — keep: coders poll ' + summonId, 'dim');
      }
    }, 5000);
  },
  ...
  async summon(task) {
    ...
    if (AciCli) {
      if (r.fallback) {
        AciCli.print('⚠ Composer down → fallback: ' + (r.via || 'llm'), 'dim');
        if (r.fallback_reason) AciCli.print('  reason: ' + r.fallback_reason, 'dim');
      }
      if (r.summon_id) {
        const eng = r.fallback ? 'fallback' : tag;
        AciCli.print('summon #' + r.summon_id + ' · ' + eng + (r.via ? ' · ' + r.via : ''), r.pending ? 'dim' : 'ok');
      }
      AciCli.print(label + ': ' + text.slice(0, 880), r.ok !== false ? 'out' : 'err');
      if (r.pending) AciCli.print('polling… type: coders poll ' + r.summon_id, 'dim');
    }
    const hud = r.fallback
      ? 'Fallback coder (' + (r.via || 'llm') + ')'
      : label + (r.summon_id ? ' #' + r.summon_id : '');
    ACIControl?.reply(hud + ': ' + text.slice(0, 150));
