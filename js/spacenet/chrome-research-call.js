/**
 * P0 RESEARCH + CALL wire — Build 20260822060000-research-call
 * - Research submit: answer in CLI within timeout; orbit pin if place; else stay on globe
 * - Non-place questions never hang / never teleport (no Milas, South America, Lagos)
...
(function (G) {
  ...
  function answerFast(line) {
    return new Promise(function (resolve) {
      ...
      if (/astranov|spacenet/.test(low)) {
        finish('Astranov is the mind. SpaceNet is the net — delivery and presence on the live globe (Earth, orbit, field). Name a place, a thing, or an order.', 'local');
        return;
      }
  ...
  function scrubAuthRunbook() {
    try {
      ...
      SNAuth.__snScrub = 1;
      var debug = owner() || (function () { try { return /[?&]sn-debug=1(?:&|$)/.test(location.search || ''); } catch (_) { return false; } })();
      if (typeof SNAuth.setupLines === 'function' && !debug) SNAuth.setupLines = function () { return []; };
      ...
      if (typeof orig === 'function') {
        SNAuth.openModal = function (err) {
          ...
          try {
            ...
            document.querySelectorAll('.sn-auth-actions, #sn-auth-setup-btn, #sn-auth-copy-btn, #sn-auth-cid').forEach(function (el) { if (el) el.style.display = 'none'; });
  ...
  function init() {
    ...
    document.addEventListener('sn:os-ready', function () {
      wireSpaceLinks().then(function () { patchCallGuest(); patchCliResearch(); });
    });
    setTimeout(function () { wireSpaceLinks(); patchCliResearch(); patchCallGuest(); }, 2500);
  }
