/* SpaceNet 4227 — guest AV€ + real Earth (land-rings) + Earth online */
(function (global) {
  "use strict";
  ...
  function paintEarth(ctx, w, h) {
    ...
    var cx = w * 0.5, cy = h * 0.5, R = Math.min(w, h) * 0.46 / dist;
    if (!(R > 8)) return;
    ctx.save();
    try {
      ...
      ctx.strokeStyle = "rgba(126,233,255,0.35)";
  ...
  function isCyan(style) {
    var s = String(style || "");
    return s.indexOf("77,240,255") >= 0 || s.indexOf("126,233,255") >= 0;
  }
  function isOcean(style) {
    var s = String(style || "").toLowerCase();
    return s === "#041018" || s.indexOf("4,16,24") >= 0 || s.indexOf("4, 16, 24") >= 0;
  }
  ...
  function wrapContext(ctx) {
    if (!ctx || ctx.__snEarth4227) return ctx;
    ctx.__snEarth4227 = true;
    var _fill = ctx.fill.bind(ctx);
    ctx.fill = function () {
      try {
        if (ctx.canvas && ctx.canvas.id === "g") {
          if (isOcean(ctx.fillStyle)) {
            var sz = cssSize(ctx.canvas);
            paintEarth(ctx, sz.w, sz.h);
            return;
          }
    ...
    ctx.stroke = function () {
      try {
        if (ctx.canvas && ctx.canvas.id === "g" && isCyan(ctx.strokeStyle)) return;
      } catch (e) {}
      return _stroke();
    };
  ...
  function patchCanvas() {
    ...
    if (!cv || cv.__snEarthPatch4227) return;
    cv.__snEarthPatch4227 = true;
  ...
  function loadRings() {
    fetch("/js/spacenet/assets/land-rings.json?v=4227", { cache: "no-store" })
      .then(function (r) { return r.ok ? r.json() : null; })
      .then(function (rings) {
        if (rings && rings.length) {
          LAND = rings;
          global.__SN_LAND_RINGS_4227 = true;
          try {
            var cv = document.getElementById("g");
            if (cv) {
              var ctx = cv.getContext("2d");
              var sz = cssSize(cv);
              if (ctx) paintEarth(ctx, sz.w, sz.h);
            }
          } catch (e) {}
        }
      })
  ...
  function boot() {
    ...
    setTimeout(patchSay, 0);
    setTimeout(patchSay, 200);
    setTimeout(paintGuestMoney, 0);
  }

  if (document.readyState === "loading") document.addEventListener("DOMContentLoaded", boot);
  else boot();
  setInterval(function () {
    ensureCam(); patchPaintMoney(); patchSay(); paintGuestMoney(); patchCanvas();
  }, 1000);
