...
(function (global) {
  ...
  function paintNairobiPins(place, q) {
    ...
    try { if (global.SN) SN.lastHunt = global.__SN_LAST_HUNT; } catch (e) {}
    try { if (SN.showCity) SN.showCity(place); } catch (e) {}
    try { if (SN.showMap) SN.showMap(list[0], 14); } catch (e) {}
    try { if (SN.paintHuntPins) SN.paintHuntPins(list, place, true); } catch (e) {}
    ...
    SN.talk = function (msg) {
      ...
      if (/Pick:\s*my location,\s*vendor/i.test(s)) {
        ...
        if (place && !placeIsRhodes(place)) {
          paintNairobiPins(place, s.split(".")[0] || "pizza");
          return;
        }
      }
      return ot(msg);
