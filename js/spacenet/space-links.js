 ...
 * Build: 20260822060000-space-links
 */
(function (global) {
  'use strict';
  var BUILD = '20260822060000-space-links';
  ...
  function addCall(peer, opts) {
    ...
    links[id] = { type: 'call', live: true, a: a, b: b, line: line, pinA: pA, pinB: pB, color: color, label: opts.label || 'Call', born: Date.now() };
    try {
      if (SNGlobe.frameRoute) SNGlobe.frameRoute([a, b], { tier: opts.tier || 'national' });
    } catch (_) {}
    log('CALL on globe · ' + (opts.label || 'peer'), 'ok');
    return id;
  }
  ...
  function addResearch(place, opts) {
    ...
    links[id] = { type: 'research', live: true, a: mePos(), b: place, line: line, pinB: p, color: color, label: opts.label || 'Research', born: Date.now() };
    log('RESEARCH pin · ' + (opts.label || place.lat.toFixed(2)), 'ok');
    return id;
  }
  ...
  function addOrderRoute(from, to, opts) {
    ...
    links[id] = { type: 'order', live: true, a: from, b: to, line: line, color: color, label: opts.label || 'Order', born: Date.now() };
    try {
  ...
  function addFieldPin(place, opts) {
    ...
    links[id] = { type: 'field', live: true, b: place, pinB: p, color: color, label: opts.label || 'Field', born: Date.now() };
    return id;
  }
  ...
  function resolvePlace(q) {
    return new Promise(function (resolve) {
      ...
      var seeds = {
        ...
        kalithea: { lat: 36.3876, lng: 28.2225, label: 'Kalithea' },
