...
- **Wrap the wordmark in a box.** The "ASTRANOV" name (`#app-name`) is
  rounded geometric letters (Quicksand) floating in space — pure glow,
  nothing else. BANNED: any background fill, border, backdrop blur,
  rounded badge, box-shadow halo, orange or gold trim, animation
  pulse, or any styling that makes the name look like a button. The
  glow is **deep electric blue** — white core → `#4aa6ff` →
  `#1e6cff` → `#0040d8` → `#001a66`. The name is always visible;
  no opacity / display / visibility toggle may hide it.
