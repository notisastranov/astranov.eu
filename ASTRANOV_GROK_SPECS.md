...
**Date recycled:** 2026-07-10 (SpaceNet mission solidified — all sessions, base lock)  
**Mission doc:** ASTRANOV_SPACENET_MISSION.md (read first)
...
## 15. SpaceNet Mission (2026-07-10 — solidified)

**Astranov SpaceNet System:** unify all internet and code under realistic space imagery and evolve internet → SpaceNet.

**Navigation tiers (one camera, continuous zoom):** SOLAR → GLOBAL → NATIONAL → CITY → STREET/PERSONAL  
**Modules:** `CosmicZoom`, `ZoomTiers`, `CityMap`, `GlobeEntity`, `Commerce`, `ACI`  
**Realism:** procedural `AIGraphics` + real geocoding/routing/WebRTC — no simulation theater

## 16. Base Version Lock (never destroy)

- Deploy artifact: monolithic `index.html` (~570KB), **not** bootstrap shell + `/src/*.js` tags
- Vercel: no `bootstrap-index.mjs` in build
- **Required CSS:** `#aci-hud { position:fixed; bottom:0 }` + `#globe-deck { display:flex }` — deletion = app death
- **Primary UI:** `#globe-deck` CLI (bottom). Celestial Circles (floating balls) **DISABLED** per owner 2026-07-10
- **Forbidden:** `simulateACI`, extra inline scripts, troll titles (MILKED/RESTORED), hiding globe-deck via JS
- **Pre-push:** `node scripts/guard-base.mjs` must pass

## 17. Session recycling (all users, all agents)

1. Stop on disaster or vision change  
2. Update ASTRANOV_SPACENET_MISSION.md + this SPECS + CLAUDE.md  
3. Implement only from updated contract  
4. Guard + push + deploy astranov.eu  

— Recycled for Notis Astranov / Astranov SpaceNet (app name always English letters)
